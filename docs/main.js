...
function createElement(tag, parentNode) {
  ...
  if (parentNode) parentNode.appendChild(svgElement);
  return svgElement;
}
...
function animateElementRemoval(svgElements, panels) {
  for (const svgElement of svgElements) animateVisibility(svgElement, false);
  window.setTimeout(
    function() {
      for (const svgElement of svgElements)
        svgElement.parentNode.removeChild(svgElement);
      if (panels) for (const panel of panels) this.updatePanelSockets(panel);
    }.bind(this),
    300
  );
}
...
function setupEventListeners(node) {
  const mousedown = function(event) {
    ...
    const mousePos = this.mousePositionOfEvent(
      event.touches ? event.touches[0] : event
    );
    if (event.shiftKey) this.setSelected([node], 'toggle');
    else {
      ...
      switch (node.type) {
        ...
        case 'socket':
          if (
            !this.eventListeners.wireDrag ||
            !this.eventListeners.wireDrag(node)
          )
            break;
          this.dragging = {
            srcSocket: node,
...
function deleteSelected(event) {
  ...
  let callback;
  if (this.eventListeners.remove) callback = this.eventListeners.remove();
...
class WiredPanels {
  constructor(parentElement, config = {}, eventListeners = {}) {
    const acceptClipboard = function(event) {
      if (!this.eventListeners.acceptClipboard) return false;
      event.stopPropagation();
    ...
    const copy = function(event) {
      if (!this.eventListeners.copy) return false;
      event.stopPropagation();
      if (!this.eventListeners.copy(event.clipboardData)) return false;
      event.preventDefault();
      return true;
    }.bind(this);
    const cut = function(event) {
      if (copy(event)) deleteSelected.call(this);
    }.bind(this);
    ...
    const drop = function(event) {
      if (acceptClipboard(event)) this.eventListeners.paste(event.dataTransfer);
    }.bind(this);
    const dragover = function(event) {
      if (acceptClipboard(event)) event.dataTransfer.dropEffect = 'copy';
    }.bind(this);

    const keydown = function(event) {
      if (
        this.svg.parentNode.querySelector('svg:hover') === null ||
        event.ctrlKey
      )
        return;
    ...
    const mousemove = function(event) {
      ...
      if (this.dragging === this.boxSelection) {
        this.boxSelection.minX = Math.min(
          this.boxSelection.originX,
          mousePos[0]
        );
        this.boxSelection.minY = Math.min(
          this.boxSelection.originY,
          mousePos[1]
        );
        this.boxSelection.maxX = Math.max(
          this.boxSelection.originX,
          mousePos[0]
        );
        this.boxSelection.maxY = Math.max(
          this.boxSelection.originY,
          mousePos[1]
        );
        this.boxSelection.setAttribute('x', this.boxSelection.minX);
        this.boxSelection.setAttribute('y', this.boxSelection.minY);
        this.boxSelection.setAttribute(
          'width',
          this.boxSelection.maxX - this.boxSelection.minX
        );
        this.boxSelection.setAttribute(
          'height',
          this.boxSelection.maxY - this.boxSelection.minY
        );
    ...
    const mouseup = function(event) {
      ...
      if (
        !event.shiftKey &&
        (!this.dragging || this.dragging === this.boxSelection)
      )
      ...
      if (this.dragging === this.boxSelection) {
        ...
        if (this.draggingMoved) {
          const isSocketInBoxSelection = function(socket) {
            return (
              this.boxSelection.minX < socket.primaryElement.x &&
              socket.primaryElement.x < this.boxSelection.maxX &&
              this.boxSelection.minY < socket.primaryElement.y &&
              socket.primaryElement.y < this.boxSelection.maxY
            );
          }.bind(this);
          const nodes = new Set();
          for (const panel of this.panels) {
            ...
            if (
              this.boxSelection.minX < rect[0] &&
              rect[1] < this.boxSelection.maxX &&
              this.boxSelection.minY < rect[2] &&
              rect[3] < this.boxSelection.maxY
            )
              nodes.add(panel);
            for (const socket of panel.sockets)
              if (isSocketInBoxSelection(socket)) nodes.add(socket);
          }
          for (const wire of this.wires)
            if (
              isSocketInBoxSelection(wire.srcSocket) &&
              isSocketInBoxSelection(wire.dstSocket)
            )
              nodes.add(wire);
    ...
    while (parentElement.getElementsByClassName('fallback').length > 0)
      parentElement.removeChild(
        parentElement.getElementsByClassName('fallback')[0]
      );
    ...
    const feComponentTransfer = createElement(
      'feComponentTransfer',
      blurFilter
    );
  ...
  setSelected(nodes, selectionMode) {
    for (const node of nodes) {
      ...
      if (selectionMode === wasSelected) continue;
      if (selectionMode === 'toggle') selectionMode = !wasSelected;
      if (selectionMode) {
  ...
  updatePanelSockets(panel) {
    ...
    panel.topSeparator.setAttribute(
      'opacity',
    ...
    panel.middleSeparator.setAttribute(
      'opacity',
      panel.label.textContent.length > 0 &&
      panel.leftSockets.length +
        panel.rightSockets.length +
        panel.bottomSockets.length >
        0
        ? 1
        : 0
    );
    panel.bottomSeparator.setAttribute(
      'opacity',
  ...
  updatePanelGeometry(panel) {
    const topAndBottomLine = this.config.socketsOutside ? 1 : 1.75,
      ...
      totalLine = Math.max(
        topLine +
          nameLine +
          Math.max(panel.leftSockets.length, panel.rightSockets.length) +
          bottomLine,
        1
      );
    for (const socket of panel.sockets)
      socket.label.width = Math.max(
        this.config.panelPadding,
        socket.label.getComputedTextLength()
      );
    let topLineWidth = this.config.panelPadding * (panel.topSockets.length + 1),
      bottomLineWidth =
        this.config.panelPadding * (panel.bottomSockets.length + 1);
    for (const socket of panel.topSockets) topLineWidth += socket.label.width;
    for (const socket of panel.bottomSockets)
      bottomLineWidth += socket.label.width;
    panel.width = Math.max(
      topLineWidth,
      bottomLineWidth,
      panel.label.getBBox().width + this.config.panelPadding * 2
    );
    for (let i = 0; i < doubleLineCount; ++i)
      panel.width = Math.max(
        panel.width,
        this.config.panelPadding * (horizontalSocketPadding * 2 - 1) +
          panel.leftSockets[i].label.width +
          panel.rightSockets[i].label.width
      );
    for (let i = doubleLineCount; i < panel.leftSockets.length; ++i)
      panel.width = Math.max(
        panel.width,
        this.config.panelPadding * horizontalSocketPadding +
          panel.leftSockets[i].label.width
      );
    ...
    const verticalSockets = function(sockets, lineWidth, sideFactor) {
      let posX = (panel.width - lineWidth) / 2,
        lastWidth = 0;
      const posY = Math.round(
          (sideFactor === -1 ? panel.height : 0) -
            sideFactor *
              this.config.panelPadding *
              (this.config.verticalSocketsOutside ? 1 : -1)
        ),
        labelPosY = Math.round(
          sideFactor *
            this.config.panelPadding *
            (this.config.socketsOutside ? 2 : 1.5)
        );
    ...
    const horizontalSockets = function(sockets, sideFactor) {
      const paddingFactor = this.config.horizontalSocketsOutside ? 1 : 0,
        posX = Math.round(
          (sideFactor === -1 ? panel.width : 0) +
            sideFactor * (1 - 2 * paddingFactor) * this.config.panelPadding
        ),
        labelPosX = Math.round(
          sideFactor * (1 + paddingFactor) * this.config.panelPadding
        );
      for (let i = 0; i < sockets.length; ++i) {
        const socket = sockets[i];
        socket.x = posX;
        socket.y =
          ((topLine + nameLine + i) * 2 + 1) * this.config.panelPadding;
        socket.label.setAttribute('x', labelPosX);
        socket.label.setAttribute('y', 0);
        socket.label.setAttribute(
          'text-anchor',
          sideFactor === 1 ? 'start' : 'end'
        );
      }
    }.bind(this);
    ...
    for (const socket of panel.sockets)
      socket.group.setAttribute(
        'transform',
        'translate(' + Math.round(socket.x) + ', ' + Math.round(socket.y) + ')'
      );
    panel.topSeparator.setAttribute('width', panel.width);
    panel.topSeparator.setAttribute(
      'y',
      Math.round(topLine * this.config.panelPadding * 2)
    );
    panel.middleSeparator.setAttribute('width', panel.width);
    panel.middleSeparator.setAttribute(
      'y',
      Math.round((topLine + 1) * this.config.panelPadding * 2)
    );
    panel.bottomSeparator.setAttribute('width', panel.width);
    panel.bottomSeparator.setAttribute(
      'y',
      Math.round((totalLine - bottomLine) * this.config.panelPadding * 2)
    );
  ...
  stabilizeGraph() {
    ...
    const tickGraph = function(currentTime) {
      ...
      if (this.animationRunning) window.requestAnimationFrame(tickGraph);
      ...
      if (this.config.springStiffness > 0)
        for (const spring of this.springs) {
          ...
          const distance = Math.max(1, Math.sqrt(vecX * vecX + vecY * vecY)),
            ...
            srcPanelIsFixed =
              draggingPanels && this.dragging.has(spring.srcPanel),
            dstPanelIsFixed =
              draggingPanels && this.dragging.has(spring.dstPanel),
            factor =
              this.config.springStiffness *
              displacement /
              distance *
              (srcPanelIsFixed || dstPanelIsFixed ? 2 : 1);
          vecX *= factor;
          vecY *= factor;
          if (!srcPanelIsFixed) {
      ...
      if (this.config.panelCollision) {
        ...
        for (const panelA of this.panels) {
          ...
          for (const panelB of this.panels) {
            if (i <= j) break;
            const rectA = this.boundingRectOfPanel(panelA),
              rectB = this.boundingRectOfPanel(panelB);
            let overlapX =
                Math.min(rectA[1], rectB[1]) - Math.max(rectA[0], rectB[0]),
              overlapY =
                Math.min(rectA[3], rectB[3]) - Math.max(rectA[2], rectB[2]);
            if (overlapX <= 0 || overlapY <= 0) continue;
            if (Math.abs(overlapX) < Math.abs(overlapY)) {
              if (panelA.x < panelB.x) overlapX *= -1;
              panelA.x += overlapX;
              panelB.x -= overlapX;
            } else {
              if (panelA.y < panelB.y) overlapY *= -1;
              panelA.y += overlapY;
              panelB.y -= overlapY;
            }
            ++j;
      ...
      if (this.config.borderCollision) {
        ...
        for (const panel of this.panels) {
          ...
          if (panelRect[0] < 0) panel.x -= panelRect[0];
          else if (panelRect[1] > rect.width)
            panel.x -= panelRect[1] - rect.width;
          if (panelRect[2] < 0) panel.y -= panelRect[2];
          else if (panelRect[3] > rect.height)
            panel.y -= panelRect[3] - rect.height;
        }
      }
      for (const panel of this.panels) tickPanel(panel);
      for (const wire of this.wires) tickWire.call(this, wire);
    }.bind(this);
    tickGraph(lastTime);
  }
  ...
  undoableAction(action) {
    ...
    if (
      this.config.undoActionLimit > 0 &&
      this.actionStack.length > this.config.undoActionLimit
    ) {
      this.actionStack = this.actionStack.slice(
        this.actionStack.length - this.config.undoActionLimit
      );
  ...
  changeGraphUndoable(nodesToAdd, nodesToRemove, callback) {
    this.undoableAction(
      function(forward) {
        ...
        if (forward) this.changeGraph(nodesToAdd, nodesToRemove);
        else this.changeGraph(nodesToRemove, nodesToAdd);
      }.bind(this)
    );
  }
  ...
  changeGraph(nodesToAdd, nodesToRemove) {
    ...
    for (const node of nodesToAdd) {
      ...
      switch (node.type) {
        case 'socket':
          ...
          if (node.index === undefined) node.panel.sockets.push(node);
          else node.panel.sockets.splice(node.index, 0, node);
        ...
        case 'wire':
          ...
          if (
            !connectSocket(node.srcSocket, node.dstSocket.panel) ||
          ...
          if (node.srcSocket.panel !== node.dstSocket.panel) {
            ...
            if (spring) ++spring.referenceCount;
            else {
              spring = {
                referenceCount: 1,
    ...
    const deleteSocket = function(socket) {
      for (const [panel, wires] of socket.wiresPerPanel)
        for (const wire of wires) nodesToRemove.add(wire);
    }.bind(this);
      ...
      switch (node.type) {
        ...
        case 'wire':
          ...
          function disconnectSocket(srcSocket, dstPanel) {
            if (!srcSocket.wiresPerPanel.has(dstPanel)) return false;
            const set = srcSocket.wiresPerPanel.get(dstPanel);
            if (!set.has(node)) return false;
            set.delete(node);
            if (set.size === 0) srcSocket.wiresPerPanel.delete(dstPanel);
            return true;
          }
          if (
            !disconnectSocket(node.srcSocket, node.dstSocket.panel) ||
            !disconnectSocket(node.dstSocket, node.srcSocket.panel)
          )
            console.error('Wire was already disconnected', node);
          if (node.srcSocket.panel !== node.dstSocket.panel) {
            const spring = node.srcSocket.panel.springs.get(
              node.dstSocket.panel
            );
            if (spring.referenceCount > 1) --spring.referenceCount;
            else {
    ...
    for (const panel of panelsToUpdateA) this.updatePanelSockets(panel);
...
async function loadModule(id, version, depth) {
  ...
  const result = await fetch(`${proxy}https://registry.npmjs.org/${id}`);
